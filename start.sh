...
# the sega user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the sega user to the desired the user (NB_USER)
    if id sega &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" sega 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the sega user:"
            _log "- username: sega       -> ${NB_USER}"
            _log "- home dir: /home/sega -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the sega user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the sega home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "sega" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/sega to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/sega/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/sega to /home/${NB_USER}!"
                _log "Attempting to symlink /home/sega to /home/${NB_USER}..."
                if ln -s /home/sega "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/sega to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/sega/"* ]]; then
...
# user we started as.
    ...
    SEGA_UID="$(id -u sega 2>/dev/null)"  # The default UID for the sega user
    SEGA_GID="$(id -g sega 2>/dev/null)"  # The default GID for the sega user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old sega user to nayvoj ($(id -u sega):$(id -g sega))"
            ...
            sed --expression="s/^sega:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/sega:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "sega" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/sega. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/sega ]]; then
        _log "WARNING: no write access to /home/sega. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
